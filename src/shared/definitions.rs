...
/// Urgent or out-of-band data is available.
/// Linux >= 2.5.44
...
/// An error condition occurred on the associated file descriptor.
///
/// Linux reports this event whether or not it was requested.
///
/// Linux >= 2.5.44
pub const EPOLLERR: u32 = 0x0000_0008;

/// A hangup occurred on the associated file descriptor.
///
/// Linux reports this event whether or not it was requested.
///
/// Linux >= 2.5.44
...
/// An invalid polling request occurred.
/// Linux >= 4.16
pub const EPOLLNVAL: u32 = 0x0000_0020;

/// Normal priority data is available for reading.
/// Linux >= 2.5.44
pub const EPOLLRDNORM: u32 = 0x0000_0040;

/// Priority band data is available for reading.
/// Linux >= 2.5.44
...
/// The stream peer closed the connection or shut down its writing half.
/// Linux >= 2.6.17
pub const EPOLLRDHUP: u32 = 0x0000_2000;

/// Wakeup generated by `io_uring`.
///
/// Linux describes this as an internal bit
/// and uses it to detect recursion back into
/// the `io_uring` poll handler.
///
/// Linux >= 6.2
pub const EPOLL_URING_WAKE: u32 = 1 << 27;

/// Use exclusive wakeups for the target file descriptor.
/// Linux >= 4.5
pub const EPOLLEXCLUSIVE: u32 = 1 << 28;

/// Request wakeup handling that inhibits system suspend.
///
/// Linux clears this bit from the copied event unless Linux is built
/// with `CONFIG_PM_SLEEP` and the calling task has `CAP_BLOCK_SUSPEND`.
///
/// Linux >= 3.5
pub const EPOLLWAKEUP: u32 = 1 << 29;

/// Disable event reporting after reporting one event.
/// Linux >= 2.6.2
pub const EPOLLONESHOT: u32 = 1 << 30;

/// Use edge-triggered instead of level-triggered notification.
/// Linux >= 2.5.44
pub const EPOLLET: u32 = 1 << 31;

/// Add a file descriptor to an epoll interest set.
/// Linux >= 2.5.44
pub const EPOLL_CTL_ADD: i32 = 1;

/// Remove an item from an epoll interest set.
/// Linux >= 2.5.44
pub const EPOLL_CTL_DEL: i32 = 2;

/// Change the event settings of an existing epoll item.
/// Linux >= 2.5.44
