...
This function performs the `epoll_pwait2` system call
...
When `timeout` is null, Linux waits indefinitely.

When `timeout` is not null, it points to a
[`__kernel_timespec`](crate::definitions::__kernel_timespec)
specifying a relative timeout in seconds and nanoseconds.
Linux copies the structure and requires `tv_sec` to not be
negative and `tv_nsec` to be between zero and 999,999,999
inclusive.

A timeout of zero seconds and zero nanoseconds
checks the ready list without blocking. Any other
valid timeout waits until Linux finds a ready event,
the calling task is interrupted, or the timeout expires.

Linux adds a nonzero timeout to the current monotonic time
to form an absolute deadline. If that addition overflows,
Linux clamps the deadline instead of rejecting the timeout.
Linux computes timer slack for the wait and may allow its
high-resolution timer to expire after the deadline so timer
wakeups can be coalesced. Scheduling may further delay
the resumption of the calling task.

Linux does not modify or retain `timeout`
...
Linux may return:
 ...
 - [`-EFAULT`](crate::Errno::EFAULT)
   ...
   A non-null `timeout` could not be read,
 ...
 - [`-EINVAL`](crate::Errno::EINVAL)
   ...
   `timeout` is non-null and `tv_sec` is negative
   or `tv_nsec` is less than zero or greater than
   999,999,999, `signal_mask` is non-null and
   `signal_mask_size` does not equal `sizeof(sigset_t)`,
   `max_events` is not positive or exceeds `EP_MAX_EVENTS`,
   or `epoll` is open but does not refer to an epoll instance.

Linux processes a non-null `timeout` before every signal mask
or epoll wait argument. It copies the `__kernel_timespec` before
validating its fields, so timeout `-EFAULT` takes precedence over
a timeout `-EINVAL`. Either timeout error takes precedence over
errors in `signal_mask`, `epoll`, `events`, or `max_events`.

After accepting the timeout, Linux processes a non-null
signal mask before examining any epoll wait argument.
It validates `signal_mask_size` before reading through
`signal_mask`, so a signal mask `-EINVAL` takes precedence
over a signal mask `-EFAULT`. Signal mask errors take
precedence over errors in `epoll`, `events`, or `max_events`.
...
`timeout`, or `signal_mask` respects the Rust
memory model and all other invariants in Rust
or anywhere else.
...
Linux reads a non-null `timeout` only while copying
the `__kernel_timespec` before processing the other
arguments. It is not accessed after that point.
...
None of the pointers need to be dereferenceable
