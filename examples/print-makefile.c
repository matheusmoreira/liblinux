...
void _start(void) {
    ...
    do {
        ...
        bytes_written = write(standard_output, buffer, bytes_read);
...
#define function "open"
    ...
    error_message(EINTR, "Open system call interrupted by a signal");
    error_message(EINVAL, "Invalid open flags");
    error_message(EISDIR, "Path refers to a directory");
    error_message(ELOOP, "Too many symbolic links encountered while resolving path");
...
#define function "read"
    ...
    error_message(EINTR, "Read system call interrupted by a signal before any data was read");
...
#define function "write"
    ...
    error_message(EINTR, "Write system call interrupted by a signal before any data was written");
